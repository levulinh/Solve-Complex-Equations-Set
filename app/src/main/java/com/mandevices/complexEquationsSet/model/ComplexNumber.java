...
public class ComplexNumber {
        ...
        if (s.contains("a") || s.contains("A")) {
            ...
            if (!s.contains("r")) {
                ...
                if (r >= 0) {
                    ...
                    return new ComplexNumber(-r * Math.cos(phi), -r * Math.sin(phi)).opposite();
                }
            } else {
                s = s.replaceAll("r", "");
                parts = s.split("a");
    ...
    public ComplexNumber divide(ComplexNumber z) {
        ...
        return z1.multiply(z.conjugate().divide(
                z.getRealPart()
                        * z.getRealPart()
                        + z.getImaginaryPart()
                        * z.getImaginaryPart()
    ...
    public String toString() {
        ...
        if (this.imaginaryPart < -eps)
            return (float) this.realPart + "" + (float) this.imaginaryPart + "i";
        
        if (this.imaginaryPart > -eps && this.imaginaryPart < eps)
            return (float) this.realPart + "";
        
        return (float) this.realPart + "+" + (float) this.imaginaryPart + "i";
    }
    
    public void cloneFrom(ComplexNumber z) {
    ...
    public String toPolarRadian() {
        ...
        return (float) r + "∠" + ((phi < 0) ? (float) phi : (float) phi + 360f) + "r";
    }
}
